...
// Hive에 StatModel 등록하기
// 절대 중복되어서는 안된다!
@HiveType(typeId: 1)
// 모델링을 할 때는 클래스로 데이터를 만든다.
class StatModel {
  @HiveField(0)
  final double daegu;
  @HiveField(1)
  ...
  // Hive Type에서는 기본 생성자를 꼭 만들어줘야 한다.
  StatModel({
    required this.daegu,
  ...
  static ItemCode parseItemCode(String raw) {
    ...
    // enum값을 출력할 때는 enum.NO2.name; 식으로 써주어야 한다.
    // toString()을 쓰게되면 ItemCode.NO2 가 그대로 출력된다.
